...
module.exports = __webpack_require__("7OQe");
...
/* harmony default export */ __webpack_exports__["default"] = (Object(_src_apiHelpers__WEBPACK_IMPORTED_MODULE_0__[/* multiRouteHandler */ "c"])('item', {
...
async function getUserCohorts(req) {
  let token = Object(_src_token__WEBPACK_IMPORTED_MODULE_1__[/* getToken */ "a"])(req);
  if (!token) return {
    status: 403,
    result: "Error: no user logged in"
  };
  let [course_cohorts, invited_courses, events] = await Promise.all([lib_prisma__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].course_cohorts.findMany({
    where: {
      OR: [{
        people_in_cohorts: {
          some: {
  ...
  }), lib_prisma__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].courses.findMany({
    where: {
      course_invites: {
        some: {
          email: token.email
  ...
  }), lib_prisma__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].standalone_events.findMany({
    include: {
      events: true
    },
    where: {
...
async function getUserCourses(req) {
  ...
  let [maintaining_courses, watching_courses] = await Promise.all([lib_prisma__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].courses.findMany({
    where: {
      course_maintainers: {
        some: {
          maintainer: token.id
  ...
  }), lib_prisma__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].watching_courses.findMany({
    where: {
      email: token.email
    }
  })]);
...
async function checkUsername(req) {
  ...
  let people = await lib_prisma__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].people.findFirst({
    where: {
      username: {
        equals: username,
        mode: "insensitive"
