...
const handler = async req => {
  ...
  if (person) {
    ...
    return {
      status: 200,
      headers: Object(_src_token__WEBPACK_IMPORTED_MODULE_1__[/* setTokenHeader */ "c"])(token),
      result: token
    };
  } else {
    return {
...
/* harmony default export */ __webpack_exports__["default"] = (Object(_src_apiHelpers__WEBPACK_IMPORTED_MODULE_0__[/* APIHandler */ "a"])(handler));

async function validateLogin(emailOrUsername, password) {
  try {
    let person = await lib_prisma__WEBPACK_IMPORTED_MODULE_3__[/* default */ "a"].people.findFirst({
      where: {
        OR: [{
          email: {
    ...
    if (!(await bcryptjs__WEBPACK_IMPORTED_MODULE_2___default.a.compare(password, person.password_hash))) return false;
    return person;
  } catch (e) {
    console.log(e);
    return false;
