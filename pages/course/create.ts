...
const CreateCourse = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const returnCallCreateCourse = await callCreateCourse("/api/courses", {
        ...formData,
      });
      if (returnCallCreateCourse.status !== 200) {
        errorNotify(returnCallCreateCourse.result);
      } else {
        successNotify(returnCallCreateCourse.result);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return h("div", [
    ...
    status === "success"
      ? h(
          Link,
          {
            href: `/dashboard?tab=Maintaining`,
          },
          "Course created!"
        )
      : null,
